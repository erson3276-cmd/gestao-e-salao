...
import { ShieldCheck, ArrowRight, Building2, Phone, Loader2 } from 'lucide-react'
    ...
    try {
      const res = await fetch('/api/auth/register', {
        ...
        body: JSON.stringify({
          ...
          ownerName: name,
          ownerEmail: email,
          ownerPassword: `google_${email}_${Date.now()}`,
      ...
      <div className="max-w-md w-full space-y-8 animate-in fade-in zoom-in-95 duration-700">
        <div className="text-center space-y-4">
          ...
          <div>
            ...
            <p className="text-[10px] uppercase font-bold tracking-[0.4em] text-gray-500 mt-2">Complete seu Cadastro</p>
          </div>
        </div>
        ...
        <div className="bg-[#121021]/50 border border-white/5 rounded-[3rem] p-10 shadow-3xl backdrop-blur-xl relative overflow-hidden">
          ...
          <div className="relative z-10">
            <div className="p-4 bg-[#5E41FF]/10 border border-[#5E41FF]/20 rounded-2xl mb-6">
              <p className="text-sm text-[#5E41FF] font-bold">Logado como: {email}</p>
            </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-5" autoComplete="off">
              <div className="space-y-2">
                ...
                <div className="relative group">
                  ...
                  <input type="text" value={salonName} onChange={(e) => setSalonName(e.target.value)} placeholder="Ex: Studio Beleza" required autoFocus className="w-full pl-14 pr-6 py-4 bg-black/40 border border-white/5 rounded-2xl outline-none focus:border-[#5E41FF]/50 focus:ring-1 focus:ring-[#5E41FF]/20 transition-all text-white placeholder:text-gray-800" />
                </div>
              </div>
